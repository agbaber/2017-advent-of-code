...
# irb(main):1122:0> @results.first
# => ["@js", "+", 257, "@wn", "<", 9]

@vals.uniq.each do |init|
  instance_variable_set(init, 0)
end

max_vals = []
@results.each do |process|
  if instance_variable_get(process[3]).public_send(process[4], process[5])
    result = instance_variable_get(process[0]).public_send(process[1], process[2])
    instance_variable_set(process[0], result)
    max_vals << result
  end
end
...
uniq_vals = []
@vals.each do |val|
  uniq_vals << instance_variable_get(val)
end
