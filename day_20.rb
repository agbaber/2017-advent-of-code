...
@input.chomp.split("\n").each_with_index do |input,i|
  ...
  input.split(', ').each do |ins|
    ...
    vals[0] = vals[0][1..-1]
    vals[-1] = vals[-1][0..-2]
    mapped = vals.map(&:to_i)
    hash_vals[key] = mapped
  end
  @vectors[i] = hash_vals
end
...
1000.times do
  @positions = {}
  @vectors.each_with_index do |vector, ind|
    vector[1]['v'].each_with_index do |velocity,i|
      vector[1]['v'][i] += vector[1]['a'][i]
    end

    vector[1]['p'].each_with_index do |position,i|
      vector[1]['p'][i] += vector[1]['v'][i]
    end

    # vector[1]['distances'] << vector[1]['p'].dup.map(&:abs).inject(:+)
    @positions[vector[0]] = vector[1]['p'].dup
  end

  deletes = {}
  deletes = @positions.group_by{|k,v| v}.select{|k,v| v.size > 1}

  if !deletes.empty?
    deletes.each do |foo|
      foo[1].map(&:first).each do |delete|
...
#   averages << (vector[1]['distances'].inject{ |sum, el| sum + el }.to_f / vector[1]['distances'].size)
# end

# averages.find_index(averages.min)
...
asked it to simulate too many particles, and it won't be able to finish them all in time to render
...
number line and diagram of current X positions (marked in parenthesis), the following would take
place:
...
that collide. Particles collide if their positions ever exactly match. Because particles are updated
