...
def aggregate_children_amounts(children)
  ...
  children.each do |child|
    sum += aggregate_child_amounts(@result[child])
  end
  sum || 0
end

def aggregate_child_amounts(child)

  nested_children = child[:stacked]
  weight = child[:weight]
  total_weight = 0

  total_weight += weight

  if nested_children.size > 0
    total_weight += aggregate_children_amounts(nested_children)
  end

  child[:combined_weight] = total_weight
  total_weight
end
...
{:weight=>4435, :stacked=>["brjneo", "nsqlbvb", "vrfek", "hxdrc", "wfqul"], :combined_weight=>69875}
{:weight=>43349, :stacked=>["rpyji", "uukembw", "pbthb"], :combined_weight=>69875}
{:weight=>31721, :stacked=>["ogyypi", "hercuw", "lnwutyi", "vxzcqpq", "fwlhy", "vyzfw"], :combined_weight=>69875}
{:weight=>55, :stacked=>["dcaxloo", "ovpoqt", "bvrxeo", "osgijzx", "uppcjl"], :combined_weight=>69881}
=> ["xdpxpu", "uewmev", "awrwywl", "hwezjo", "qqqxyrl", "luralcy"]



["xdpxpu", "uewmev", "awrwywl", "hwezjo", "qqqxyrl", "luralcy"].each do |w|
  total = @result[w][:weight] + @result[w][:combined_weight]
  puts "#{total}"
end

["dcaxloo", "ovpoqt", "bvrxeo", "osgijzx", "uppcjl"].each do |w|
  total = @result[w][:combined_weight]
  puts "#{total} - #{w}"
end

["jnyexah", "ltleg", "fdnmqri", "iysgr", "dffdie", "vvgyhb"].each do |w|
  total = @result[w][:combined_weight]
  puts "#{total} - #{@result[w][:weight]} - #{w}"
end

["vnlmjy", "bscjk", "uycjw", "kzuimi", "kwlqal", "lahieha"].each do |w|
  total = @result[w][:combined_weight]
  puts "#{total} - #{@result[w][:weight]} - #{w}"
end
...
are other programs, each holding their own disc, and so on. At the very tops of these sub-sub-sub-...
-towers, many programs stand simply keeping the disc below them balanced but with no disc of their 
own.
...
those sub-towers are supposed to be the same weight, or the disc itself isn't balanced. The weight
of a tower is the sum of the weights of the programs in that tower.
