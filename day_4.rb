...
@input.split("\n").each do |password|
  ...
  h.each_value do |group|
    expected = group.size
     sorted =[]
    if expected != 1
     
      group.each do |value|
        sorted << value.chars.sort.join("")
      end
      if sorted.uniq.size == sorted.size
        unless valid_passwords.include? a
          puts "#{a} is valid"
          valid_passwords << a
        end
      else
        puts "#{a} is invalid"
        puts "#{sorted.uniq} vs #{group}"
        failed_passwords << a
      end
    else
      if sorted.uniq.size == sorted.size
...
(valid_passwords - failed_passwords).size
...
@test_input.split("\n").each do |password|
  ...
  h.each_value do |group|
    ...
    if expected != 1
      sorted =[]
      group.each do |value|
        sorted << [value.chars.sort]
      end
      if sorted.uniq.size == expected
        unless valid_passwords.include? a
          puts "#{a} is valid"
          valid_passwords << a
        end
