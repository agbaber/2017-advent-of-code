...
until result_b_matches.size == 5000000
  result_a = (generator_a * generator_a_factor) % 2147483647
  if result_a % 4 == 0
    result_a_in_binary = result_a.to_s(2)[-16..-1]
    result_a_matches << result_a_in_binary
  end

  result_b = (generator_b * generator_b_factor) % 2147483647
  if result_b % 8 == 0
    result_b_in_binary = result_b.to_s(2)[-16..-1]
    result_b_matches << result_b_in_binary
  end

  # if result_b_in_binary == result_a_in_binary
  #   matches += 1
  #   result_a_in_binary = nil
  #   result_b_in_binary = 1
  # end
  i += 1
  if i % 1000000 == 0
    puts i
  end
  generator_a = result_a
  generator_b = result_b
end

matches = 0

result_b_matches.each_with_index do |val, index|
  if val == result_a_matches[index]
    matches +=1
  end
end
...
Generator A looks for values that are multiples of 4. Generator B looks for values that are
multiples of 8. Each generator functions completely independently: they both go through values
