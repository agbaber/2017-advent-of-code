...
@input.size = 1078 # 1078 or higher or -1 or lower is outside the array

@position = 0

steps = 0
until @position < 0 || @position >= 1078
  current_value = @position
  unless @input[current_value].nil?
    @position += @input[current_value]
    if current_value >= 3
...
steps = 0
until @position < 0 || @position >= 5
  current_value = @position
  unless @test_input[current_value].nil?
    @position += @test_input[current_value]
    if @test_input[current_value] >= 3
      @test_input[current_value] -= 1
    else
      @test_input[current_value] += 1
    end
    puts @test_input.inspect
  end
  steps +=1
end
