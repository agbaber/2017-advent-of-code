...
@test_input = [['{}',1],['{{{}}}', 6],['{{},{}}', 5],['{{{},{},{{}}}}',16],['{<a>,<a>,<a>,<a>}',1],
['{{<ab>},{<ab>},{<ab>},{<ab>}}', 9], ['{{<!!>},{<!!>},{<!!>},{<!!>}}', 9],['{{<a!>},{<a!>},{<a!>},{<ab>}}', 3]]
...
def run(input)
  ...
  input.each_char do |char|
    charater_stream << char
  end

  charater_stream.pop # remove newline

  index_to_delete = []
  charater_stream.each_with_index do |char, i|
    unless index_to_delete.include? i
      if char == '!' && charater_stream[i-1] != '!'
        index_to_delete << i
        index_to_delete << (i+1).to_i
      elsif char == '!' && (index_to_delete.include?(i-1))
        index_to_delete << i
        index_to_delete << (i+1).to_i
      end
    end
  ...
  index_to_delete.uniq.sort.reverse.each do |i|
    charater_stream.delete_at(i)
  end

  clean_stream = charater_stream

  # charater_stream.each do |char|
  #   if ['{','}','<','>'].include? char
  #     clean_stream << char
  #   end
  # end
  ...
  index_to_delete = []
  deleted_chars = []
  clean_stream.each_with_index do |char, i|
    if char == '<' && !(index_to_delete.include? i)
      index_to_delete << i
      n = i+1
      foo = false
      until foo == true
  ...
  puts "level is #{level}"
  puts "score is #{score}"
  puts "Deleted #{deleted_chars.size}"
end

@test_input.each do |test_input|
  result = run(test_input[0])
  if result == test_input[1]
    puts 'test passed'
  else
    puts 'test failed'
    puts "should have been #{test_input[1]} but was #{result}"
  end
end
...
#pt 2
# 4848 too low (forgot to not remove junk)
# 12901 too high
