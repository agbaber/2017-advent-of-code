...
until 1>2 do
  max = @input.max
  position_to_distribute = @input.index(max)

  @input[position_to_distribute] = 0
  i = 1
  o = 0
  max.times do
    if !@input[position_to_distribute+i].nil?
      @input[position_to_distribute+i] += 1
      i+=1
    else
      @input[o] += 1
      o+=1
  ...
  if !@seen_configurations.include?(@input)
    @seen_configurations << @input.dup
  else
    ind = @seen_configurations.index(@input)
    @seen_configurations.size - ind
    break
  end
