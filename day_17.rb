...
def step
  if @current_position + @input < @spinlock.size
    position = @current_position + @input
    # puts "added"
  else
    position = (@current_position + @input) - @spinlock.size
    # puts "subtracted"
    if position >= @spinlock.size
      position = (@current_position + @input) % @spinlock.size
      # puts "modulo"
    end
  end
  # puts "position is #{position}, size is #{@spinlock.size}"
  @spinlock.insert(position+1, @i+1)

  # puts @spinlock.inspect
  @current_position = @spinlock.find_index(@i+1)
  @i+=1
  if @i % 100000 == 0
    puts @i
  end
...
(0), the initial state before any insertions. 0 (1): the spinlock steps forward three times (0, 0,
0), and then inserts the first value, 1, after it. 1 becomes the current position. 0 (2) 1: the
spinlock steps forward three times (0, 1, 0), and then inserts the second value, 2, after it. 2
becomes the current position. 0  2 (3) 1: the spinlock steps forward three times (1, 0, 2), and then
inserts the third value, 3, after it. 3 becomes the current position. And so on:
0
0 (1)
0 (2) 1
0  2 (3) 1
...
1512  1134  151 (2017) 638  1513  851 Perhaps, if you can identify the value that will ultimately be
after the last value written (2017), you can short-circuit the spinlock. In this example, that would
be 638.
...
assume that's what happened; it's spinning significantly faster than it was a moment ago.
